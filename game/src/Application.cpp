...
int main(void)
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		float screenRayA = playerA + PI / 2;
		glm::vec2 screenRay(cosf(screenRayA), sinf(screenRayA));
		...
		for (int x = 0; x < screenW; x++) {
			rayA = (playerA - POV / 2.0) + POV * ((float)x / screenW);
			ray.x = cosf(rayA);
			ray.y = sinf(rayA);
			...
			while (!hasHit && distance < maxDistance) {
				...
				testX = (int)(playerX + ray.x * distance);
				testY = (int)(playerY + ray.y * distance);
				if (((testY < mapH) + (testY >= 0) + (testX < mapW) + (testX >= 0)) == 4 && map[(int)testY * mapW + (int)testX] == '#') {
					hasHit = true;
				}
			}
			if (hasHit) {
				ray *= distance;
				//distance *= fabs(cos(playerA - rayA));
				distance = sqrtf(glm::dot(ray, ray) - powf(glm::dot(screenRay, ray), 2.0));
			...
			colorbit = corner ? .1 : 1.0 - (fmin(distance, maxDistance) / maxDistance);
