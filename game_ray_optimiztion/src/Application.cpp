...
inline char bigStepRay(const char* map, const int mapW, const int mapH, const float maxDistance, const glm::vec2& ray, glm::vec2& point, float& addDistance) {
	glm::vec2 delta = glm::abs(ray);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (int x = 0; x < screenW; x++) {
			...
			while (!hasHit && distance < maxDistance) {
				testing = bigStepRay(map, mapW, mapH, maxDistance, ray, curPoint, distance);
				if (testing != 0) {
					testing = smallStepRay(map, mapW, mapH, distanceStep, maxDistance, ray, curPoint, distance);
					switch (testing) {
					case '+':
					case '|':
						rayA *= -1;
